        ...
        if argument == "live" {
            return matches
                ...
                .compactMap { match -> (LiveMatch, Int)? in
                    if MatchSchedule.status(for: match, now: now).isLive {
                        let kickoff = MatchSchedule.kickoff(from: match.time, now: now) ?? now
                        return (match, Int(now.timeIntervalSince(kickoff) / 60))
                    }
                    return nil
                }
                .min { $0.1 < $1.1 }?.0
