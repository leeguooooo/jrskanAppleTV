...
| 浏览 | 全部、最近观看、关注、热门、篮球、足球分类；窄栏主客队上下排列，宽栏保持横向布局 |
| 关注 | 在详情页关注球队，列表出现「关注」分类与标记；只存本机 |
| 线路 | 真正播出后才记录成功；结合上次成功线路、起播速度和失败记录优选，近期失败的线路暂时后排，仍可手动选择 |
| 自动恢复 | 持续检查播放进度；首帧等待超过 20 秒或播放中卡住 12 秒时恢复。正常暂停不触发换线；中途断流先重连当前线路，两分钟内再次失败再换线 |
| 播放 | 支持嵌套页面、转跳、相对地址及加密播放器链路；先检查播放列表，再交给 AVPlayer 全屏播放；传输栏内可切换线路 |
| 刷新 | 赛程每 5 分钟、比分独立每 30 秒检查；分别显示更新时间。比分失败保留旧数据并提示，连续失败逐步降低请求频率，回到前台检查更新 |
| 比分 | 与状态取自同一份事件数据；足球详情显示半场比分与角球，篮球显示分差与总分；未开赛、缺值或异常值不补零 |
| 最近观看 | 只记录已成功播放的最近 10 场，保留 7 天；首页一键继续当前赛程中的最近比赛，每次重新解析线路；可在设置清除 |
